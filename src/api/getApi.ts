...
async function callAPIAndCacheResponseF(url: string, cacheTime: number = 0, retryCount: number = 3,isForceUseCache = true,rtLevel=1): Promise<object> {
  ...
  for (let attempt = 0; attempt < retryCount; attempt++) {
    ...
    try {
      ...
      if (e && (e.response.status === 404|| e.response.status === 422)){ // 找不到就是找不到，不需要重试了。如果是访问上限，错误码不会是404/422
        logger(`API`, `Failed to get JSON from "${url}". Server returned ${e.response.status} err code. No more retries will be made.`);
        throw e;
      }
      
      logger(`API`, `Failed to get JSON from "${url}" on attempt ${attempt + 1}. Error: ${e.message}`);
      if (attempt === retryCount - 1) {
        logger(`API`,`Failed to get JSON from "${url}" after ${retryCount} attempts`);
