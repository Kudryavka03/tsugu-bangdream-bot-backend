...
import { drawListByServerList, drawListWithLine } from '@/components/list';
import { drawList, drawListMerge, line } from '@/monthlyRanking/starfx/listUi';
...
import { getEventListByDisplayServerListTimeRange } from '@/types/Event';
...
async function buildMonthlyRankingDetailBlocks(monthlyRanking: MonthlyRanking, displayedServerList: Server[]) {
    ...
    list.push(await drawList({ key: 'ID', text: monthlyRanking.monthlyRankingId.toString() }));
    list.push(line);
    ...
    if (monthlyRanking.bgmFileName) {
        ...
        if (m) {
            ...
            if (song.isExist) {
                await song.initFull();
                list.push(await drawList({ key: "月榜歌曲" }));
                list.push(await drawSongInList(song, undefined, "", displayedServerList));
            } else {
                list.push(await drawList({ key: '月榜音乐', text: monthlyRanking.bgmFileName }));
            }
        } else {
            list.push(await drawList({ key: '月榜音乐', text: monthlyRanking.bgmFileName }));
        }
...
async function buildMonthlyRankingListBlocks(monthlyRankingList: MonthlyRanking[], displayedServerList: Server[]) {
    ...
    for (const monthlyRanking of monthlyRankingList) {
        ...
        const infoBlock = await drawList({ key: 'ID', text: monthlyRanking.monthlyRankingId.toString() });

        const server = displayedServerList[0] ?? getServerByName('jp');
        const timeBlock = drawListMerge([
            await drawList({ key: '开始', text: changeTimefomant(monthlyRanking.startAt[server]) }),
            await drawList({ key: '结束', text: changeTimefomant(monthlyRanking.endAt[server]) }),
...
export async function drawMonthlyRankingList(
    ...
    const image = await drawDatablock({ list: blocks });
    const buffer = await outputFinalBuffer({
        imageList: [await drawTitle('查询', '月榜列表'), image],
        useEasyBG: useEasyBG,
        text: 'Monthly Ranking',
        compress,
    });
