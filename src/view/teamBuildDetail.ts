...
export async function drawTeamBuildDetail(player: playerDetail, server: Server, useEasyBG: boolean, compress: boolean) {
    ...
    //活动属性加成
    var attributeImage = []
    attributeImage.push(await drawList({
        key: '活动加成'
    }))
    ...
    //活动角色加成
    var characterImage = []
    characterImage.push(await drawList({
        key: '活动角色加成'
    }))
    ...
    if (msg.length > 0) {
        cardIconListInList.push(await drawText({
            text: msg,
            maxWidth: widthMax
        }))
    }
    ...
    for (const key in areaItemPercent) {
        areaItemList.push(await drawList({
        ...
        switch (parseInt(key)) {
            case AreaItemType.band:
                for (const bandId in areaItemPercent[key]) {
                    const content = []
                    content.push(bandId == '1000' ? await drawText({
                        text: '全部乐队',
                        maxWidth: widthMax
                    }) : resizeImage({
            ...
            case AreaItemType.attribute:
                ...
                for (const attribute of keys) {
                    const content = []
                    content.push(attribute == '~all' ? await drawText({
                        text: '全部颜色',
                        maxWidth: widthMax
                    }) : resizeImage({
