...
import { serverNameFullList, statusName, cnMonthlyTierList } from '@/config';
...
export async function drawMonthlyRankingCutoffAll(monthlyRankingId: number, mainServer: Server, compress: boolean): Promise<Array<Buffer | string>> {
    ...
    //初始化档线列表
    const tierList = cnMonthlyTierList;
    const cutoffList: Array<MonthlyRankingCutoff> = [];
    for (const i in tierList) {
        cutoffList.push(new MonthlyRankingCutoff(monthlyRankingId, mainServer, tierList[i]));
    }
    await MonthlyRankingCutoff.initFullBatch(cutoffList);
    for (const tempCutoff of cutoffList) {
