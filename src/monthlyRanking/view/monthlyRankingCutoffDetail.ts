import { drawList, line, drawListMerge } from '@/monthlyRanking/starfx/listUi';
...
import { MonthlyRankingCutoff } from '@/monthlyRanking/types/MonthlyRankingCutoff';
import { drawMonthlyRankingCutoffChart } from '@/monthlyRanking/components/chart/monthlyRankingCutoffChart';
...
export async function drawMonthlyRankingCutoffDetail(monthlyRankingId: number, tier: number, mainServer: Server, compress: boolean): Promise<Array<Buffer | string>> {
    ...
    all.push(await drawTitle('预测线', `${ serverNameFullList[mainServer] } 月榜 ${ cutoff.tier }档线`));
    ...
    if (cutoff.status == 'in_progress') {
        ...
        tempList.push(await drawList({ key: '月榜剩余时间', text: `${ changeTimePeriodFormat(cutoff.endAt - time) }` }));
        list.push(drawListMerge(tempList));
        list.push(line);
    } else if (cutoff.status == 'ended') {
        list.push(await drawList({ key: '状态', text: statusName[cutoff.status] }));
        list.push(line);
        list.push(await drawList({ key: '最终分数线', text: cutoff.latestCutoff.ep.toString() }));
        list.push(line);
    } else {
        list.push(await drawList({ key: '状态', text: statusName[cutoff.status] }));
        list.push(line);
        list.push(await drawList({ key: '当前分数线', text: cutoff.latestCutoff.ep.toString() }));
        list.push(line);
    }
