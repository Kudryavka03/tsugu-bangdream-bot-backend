import { drawList, line } from '@/monthlyRanking/starfx/listUi';
...
export async function drawMonthlyRankingCutoffListOfRecentEvent(monthlyRankingId: number, tier: number, mainServer: Server, compress: boolean): Promise<Array<Buffer | string>> {
    ...
    const cutoffList: Array<MonthlyRankingCutoff> = monthlyRankingList.map(
        (mr) => new MonthlyRankingCutoff(mr.monthlyRankingId, mainServer, tier),
    );
    await MonthlyRankingCutoff.initFullBatch(cutoffList);
    cutoffList.reverse();

    for (const cutoff of cutoffList) {
        if (!cutoff.latestCutoff) continue;
        list.push(await drawList({
            key: cutoff.getMonthlyRankingName(mainServer),
        }));
        ...
        list.push(await drawList({ content: cutoffContent }));
        list.push(line);
    }
