...
} from '@/monthlyRanking/components/chart/monthlyRankingCutoffChart';
...
import { drawMonthlyRankingDatablock } from '@/monthlyRanking/components/dataBlock/monthlyRanking';
import { drawDottedLine } from "@/image/dottedLine";
import { drawList, drawListMerge, STARFX_PRESET_COLORS as presetColorList } from '@/monthlyRanking/starfx/listUi';
import { drawRoundedRectWithText } from "@/image/drawRect";
import { MonthlyRanking } from "@/types/MonthlyRanking";
import { getTopRatingDuringTime, getRatingByPlayer, getAverageTime } from "@/monthlyRanking/starfx/topRateHelpers";
...
export async function drawMonthlyRankingCutoffEventTop(monthlyRankingId: number, mainServer: Server, compress: boolean): Promise<Array<Buffer | string>> {
    ...
    all.push(await drawTitle('档线', `${ serverNameFullList[mainServer] } 月榜10档线`));
    ...
    //最近maxCount次分数变化
    {
        ...
        for (let i = 0; i + 1 < playerRating.length; i += 1) {
            ...
            if ((playerRating[i].value != playerRating[i + 1].value) && (playerRating[i].time - playerRating[i + 1].time < 5 * 60 * 1000)) {
                ...
                count += 1
                const timeImage = await drawList({ text: `${mid.toTimeString().slice(0, 5)}` })
                ...
                imageList.push(drawListMerge([
                    timeImage,
                    await drawList({ text: `${score}` })], widthMax / 2))
                // list.push(line)
            }
        }
        if (count == 0) {
        ...
        all.push(await drawDatablock({ list, topLeftText: day ? `玩家于day${day}的分数变化` : `最近${maxCount}次分数变化` }))
    }
    ...
    const timeList = [1, 3, 12, 24]
    {
        ...
        for (const a of timeList) {
            ...
            if (flag) {
                ...
                list.push(drawListMerge([timeImage, await drawList({ text: `${count}` }), await drawList({ text: timestamps.length <= 1 ? '-' : `${(new Date(averageTime)).toTimeString().slice(3, 8)}` }), await drawList({ text: count == 0 ? '-' : `${(sumScore / count).toFixed(2)}` })], widthMax))
            }
            list.push(line)
        }
        list.pop()
...
export async function drawMonthlyRankingTopSleepStat(monthlyRankingId: number, playerId: number, tier: number, mainServer: Server, time: number, compress: boolean) {
    ...
    all.push(await drawTitle('查睡眠', `${serverNameFullList[mainServer]}`));
    {
        const list: Array<Image | Canvas> = [];
        //名片
    ...
    const list = [];
    list.push(drawListMerge([await drawList({ key: '日期' }), await drawList({ key: '休息时段' }), await drawList({ key: '休息时长' })], widthMax));
    ...
    if (sleep.length > 0) {
        ...
        for (const { start, end } of sleep) {
            ...
            list.push(drawListMerge([
                await drawList({ text: `${formatDate(ed)}` }),
                timeImage,
                await drawList({ text: toTimeStr(diff) })
            ], widthMax))
            list.push(line)
        }
        ...
        list.push(line)
        list.push(drawListMerge([
            await drawList({ text: '总计: ' }),
            await drawList({ text: toTimeStr(Math.floor(totalSleepTime / (1000 * 60))) }),
            await drawList({ text: '平均每天: ' }),
            await drawList({ text: toTimeStr(Math.floor(24 * 60 * totalSleepTime / (playerRating[0].time - nowMonthlyRanking.startAt[mainServer]))) })
        ], widthMax))
    } else {
        list.push(drawListMerge([await drawList({ text: '数据不足' })], widthMax))
    }
    //折线图
    list.push(await drawMonthlyRankingCutOffTopSingleChart(monthlyRankingCutoffTop, false, playerId, mainServer))
    all.push(await drawDatablock({ list, topLeftText: `休息时间统计` }))
...
export async function drawMonthlyRankingTopRateRanking(monthlyRankingId: number, mainServer: Server, compress: boolean, time: number, date: Date, compareTier: number, comparePlayerUid: number) {
    ...
    //对每一个字段进行遍历
    const keys = Object.keys(top10SpeedRankingData[0]);
    for (let index = 0; index < keys.length; index++) {
        ...
        const height = [];
        for (let i = 0; i < 10; i++) {
            const img = await drawList({ text: String(top10SpeedRankingData[i][value] || '---')});
    ...
    all.push(await drawTitle('t10时速排名', `${serverNameFullList[mainServer]}`));
...
export async function drawMonthlyRankingTopTenMinuteSpeed(monthlyRankingId: number, mainServer: Server, compress: boolean = false, date: Date, time = 60, allPlayer = false) {
    ...
    const all = [];
    const widthMax = 3000;
    all.push(await drawTitle('分速表', `${serverNameFullList[mainServer]}`));
    ...
    const header: Canvas[] = await Promise.all(headerStringArray.map(async (text, idx) => {
        const playerName = idx === 0 ? null : players[idx - 1].name;
        const color = colorMap.get(playerName) || '#505050';
        return await drawList({ text, textSize: 30, color, maxWidth: 250 });
    }));

    // 分数行，榜外玩家加括号
    const ptRow: Canvas[] = [await drawList({ text: (date ?? new Date()).toLocaleDateString() })];
    for (const p of players) {
        const showBracket = allPlayer && !topTenUids.has(p.uid);
        const ptText = showBracket ? `(${p.currentPt})` : String(p.currentPt);
        ptRow.push(await drawList({ text: ptText }));
    }
    ...
    for (let tIdx = displayTimeStamps.length - 1; tIdx >= 0; tIdx--) {
        ...
        const timeImg = await drawList({ text: timeStr });
        row.push(timeImg);
        colWidths[0] = Math.max(colWidths[0], timeImg.width);

        for (const [pIdx, player] of players.entries()) {
            const val = player.speeds[tIdx] ?? "---";
            const playerColor = colorMap.get(player.name) || '#505050';
            const speedImg = await drawList({ text: val, color: val == 0 || val == "---" ? "#CCCCCC" : playerColor });
            row.push(speedImg);
...
export async function drawMonthlyRankingTopRunningStatus(monthlyRankingId: number, playerId: number, tier: number, mainServer: Server, time: number, compress: boolean) {
    ...
    all.push(await drawTitle('查稼动', `${serverNameFullList[mainServer]}`));
    {
        const list: Array<Image | Canvas> = [];
        //名片
        var userInRankings = monthlyRankingCutoffTop.getLatestRanking();
    ...
    const list = [];
    list.push(drawListMerge([
        await drawList({ key: '日期' }),
        await drawList({ key: '稼动时间' }),
        await drawList({ key: '稼动把数' }),
        await drawList({ key: '把均pt' }),
        await drawList({ key: '稼动总pt' })
    ], widthMax));
    ...
    if (run.length > 0) {
        ...
        for (const { startTime, startValue, endTime, endValue, playTimes } of run) {
            ...
            list.push(drawListMerge([
                await drawList({ text: `${formatDate(ed)}` }),
                timeImage,
                await drawList({ text: String(playTimes) }),
                await drawList({ text: String(Math.floor((endValue - startValue) / playTimes)) }),
                await drawList({ text: String(endValue - startValue) })
        ...
        list.push(drawListMerge([
            await drawList({ text: '总计: ' }),
            await drawList({ text: toTimeStr(Math.floor(totalRunTime / (1000 * 60))) }),
            await drawList({ text: '平均每天: ' }),
            await drawList({ text: toTimeStr(Math.floor(24 * 60 * totalRunTime / (playerRating[0].time - nowMonthlyRanking.startAt[mainServer]))) })
        ], widthMax))
    } else {
        list.push(drawListMerge([await drawList({ text: '数据不足' })], widthMax))
    }
    //折线图
    list.push(await drawMonthlyRankingCutOffTopSingleChart(monthlyRankingCutoffTop, false, playerId, mainServer))
    all.push(await drawDatablock({ list, topLeftText: `稼动时间统计` }))
