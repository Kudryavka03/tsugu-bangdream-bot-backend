...
//横向组合较短list，高度为最高的list，宽度不平分
export function drawListMergeWithoutWidth(imageList: Array<Canvas | Image>, maxWidth: number = 800, drawLine: boolean = false, align: "top" | "bottom" | "center" = "top"): Canvas {
    var maxHeight = 0
    for (let i = 0; i < imageList.length; i++) {
        const element = imageList[i];
        if (element && element.height > maxHeight) {
    ...
    for (let i = 0; i < imageList.length; i++) {
        ...
        //console.log(maxWidth / imageList.length)
        x += element.width+10
    }
    return canvas
}
