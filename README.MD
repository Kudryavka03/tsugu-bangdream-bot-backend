...
> 本项目做了非常多关于可能可以缩短网络IO等待上的优化，旨在为了更快的出图速度。在部分情况下可能会因为Bestdori API接口限流导致无法出图（将在未来解决），同时因为较为激进的缓存策略，对硬盘空间有一定需求，且可能会导致部分数据需要在第二次查询才会同步最新。


## 🚪 TO DO

API接口查询限流，防止因为过多的API查询请求导致Bestdori限流
